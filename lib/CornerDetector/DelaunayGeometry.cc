...
namespace AprilTags {
	...
	void DelaunayGeometry::update( void )
	{
		...
		for( auto const &tri : triangleList ) {
			...
			if( vt.size() == 3 ) {
				...
				_membership[ vt[0] ].emplace_back( _triangles.back() );
				_membership[ vt[1] ].emplace_back( _triangles.back() );
				_membership[ vt[2] ].emplace_back( _triangles.back() );
			}
		}
	...
	{
		// For point a, warp point b and c by local affine transform.  Find similarity
	...
	// std::cout << "bPt:     " << bPt << std::endl;
		...
		// Estimate similarity
		cv::Matx23f sim( MathUtil::solveSimilarity( aWarped, bWarped, aPt, bPt  ));
...
//		std::cout << "Soln: " << ans << std::endl;
		...
		cv::Matx21f r( sim * cv::Matx31f( tWarped.x, tWarped.y, 1 ));
